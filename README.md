...
- `npm install` Create a new ToBeeDun app.
- `npm start ios` -- (`react-native run-ios`) Build the iOS App (requires a MacOS computer).
- `npm start android` -- (`react-native run-android`) Build the Android App.
- `npm start web` -- (`expo start:web`) Run the website in your browser.
